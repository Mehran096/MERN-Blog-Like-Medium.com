...
const UpdateName = () => {
    ...
    const {profileErrors} = useSelector(state => state.ProfileUpdateReducer)
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
     ...
     useEffect(() => {
        ...
        if(profileErrors.length !== 0){
         profileErrors.map((err) => toast.error(err.msg));
         }
     // eslint-disable-next-line react-hooks/exhaustive-deps
     }, [profileErrors, redirect])

    //submit user name
    const userNameUpdt = (e) => {
        e.preventDefault();
        dispatch(updateNameAction({name: Name, id: _id}))
    }
    return (
